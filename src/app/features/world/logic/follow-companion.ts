...
export function stepCompanion(
  ...
  if (distance < 0.35) {
    return { ...state, position: resolveCircles(state.position, obstacles), speed: Math.max(0, state.speed - 12 * deltaSeconds) };
  }
  ...
  let position = resolveCircles({ x: state.position.x + (dx / distance) * step, z: state.position.z + (dz / distance) * step }, obstacles);
  if (Math.hypot(position.x - state.position.x, position.z - state.position.z) < step * 0.3) {
    const side = slot.angle < 0 ? -1 : 1;
    position = resolveCircles({ x: state.position.x - (dz / distance) * step * side, z: state.position.z + (dx / distance) * step * side }, obstacles);
  }
  return {
    position,
